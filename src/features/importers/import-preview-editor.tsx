...
export function ImportPreviewEditor({ row, categories, accounts, onChange, onSaveRule }: { row: ImportPreviewRow; categories: LedgerCategory[]; accounts: { id: string; name: string; color: string }[]; onChange: (patch: Partial<ImportPreviewRow>) => void; onSaveRule?: () => Promise<void> }) {
  ...
  return <section className={`rounded-2xl border p-4 shadow-sm ${row.duplicate ? "border-[#efd5c9] bg-[#fff9f6]" : invalid && row.enabled ? "border-[#f1d38b] bg-[#fffdf5]" : "border-[#e8eeee] bg-white"}`}>
    ...
    <button type="button" onClick={() => setPickerOpen(true)} disabled={row.duplicate || row.direction === "unknown"} className={`mt-4 flex w-full items-center justify-between rounded-xl border px-4 py-3.5 text-left ${selectedCategory ? "border-[#bce5df] bg-[#effaf8]" : "border-[#e2e8e8] bg-[#f8fbfb]"}`}><span><small className="block text-xs text-[#84909a]">分类{row.categorySuggestion && selectedCategory ? ` · ${row.categorySuggestion === "ai" ? "AI推荐" : row.categorySuggestion === "user_rule" ? "用户规则" : "自动推荐"}` : ""}</small><b className={`mt-1 block ${selectedCategory ? "text-[#0c6f78]" : "text-[#76818b]"}`}>{selectedCategory?.name ?? "点击选择分类"}</b></span><span className="grid size-9 place-items-center rounded-full bg-white text-lg text-[#58716f]">›</span></button>
    ...
    <label className="mt-3 block text-sm font-medium text-[#71808b]">商品说明<input value={row.productName} onChange={(event) => onChange({ productName: event.target.value })} disabled={row.duplicate} placeholder="账单中的商品或交易描述" className="mt-1.5 w-full rounded-xl bg-[#f3f6f6] px-3 py-3 text-[#303b44] outline-none" /></label>
    {onSaveRule && !row.duplicate && row.categoryId && row.direction !== "unknown" && <button type="button" disabled={savingRule || ruleSaved} onClick={() => { setSavingRule(true); void onSaveRule().then(() => setRuleSaved(true)).finally(() => setSavingRule(false)); }} className="mt-3 w-full rounded-xl border border-[#bce5df] bg-[#f3fbfa] py-2.5 text-sm font-medium text-[#0c6f78] disabled:opacity-60">{ruleSaved ? "已记住此商户分类" : savingRule ? "正在保存规则…" : "记住此商户分类"}</button>}
